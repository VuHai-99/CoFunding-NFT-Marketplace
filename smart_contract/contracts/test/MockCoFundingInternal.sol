...
contract MockCoFundingInternal is Ownable, CoFundingErrorsAndEvents, ArrayHelpers, ReentrancyGuard  {
    ...
    function _endFundingPhase(bytes32 vaultID, uint boughtPrice)
        ...
        if(boughtPrice <= vaultInfo.initialPrice){
            ...
            vaultInfo.boughtPrice = boughtPrice;
...
/**
     ...
     */
    function _finishVault(bytes32 vaultID, uint sellingPrice)
        internal{
        ...
        if(vaultInfo.vaultState == VaultState.FUNDED){
            vaultInfo.sellingPrice = sellingPrice;
     ...
     *          +) Surplus_Money:
                    ...
                    - Surplus_Money = contributionAmount - boughtPrice
     *          +) Sold_NFT_Money:
                    - Money getting back from sold NFT. Divide according 
                      to % of contribution
                    - Sold_NFT_Money = sellingPrice
     * @param vaultID ID of selected vault.
     */
    function _divideRewardInFinishedVault(bytes32 vaultID)
        ...
        address[] memory vaultUser = _vaultUsers[vaultID];
        //Refund to user's spending wallet all money deposited into vault
        uint rewardMoney = _vaultInfos[vaultID].totalAmount + _vaultInfos[vaultID].sellingPrice - _vaultInfos[vaultID].boughtPrice;
        for(uint i = 0; i< vaultUser.length; i++){
